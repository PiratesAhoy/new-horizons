...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			...
			{
				link.l1 = DLG_TEXT[3] + GetMyFullName(PChar) + DLG_TEXT[8];
				link.l1.go = "sorry";
			}
		...
		case "fight":
			...
			link.l1 = RandSwear() + DLG_TEXT[14];
			link.l1.go = "Exit_fight";	// ccc oct06 someone put "exit" in here which wouldn't trigger fight
		break;
		...
		case "Exit_fight":
			...
			// ccc oct06 someone created this case without these exiting commands:
			DialogExit();
		...
		case "Exit_raid":
			DialogExit();
			Diag.CurrentNode = Diag.TempNode;
			// Julian: for avoiding landingboat location -->
			if(GetAttribute(Characters[GetMainCharacterIndex()],"location") == "Philipsburg_Port")
			{LAi_CreateFantomGroup("Soldiers", 5, LAI_GROUP_ENEMY, LAI_GROUP_ENEMY, FRANCE, OFFIC_TYPE_GUARD, 0, true, "", "", "", "reload2");}
			else{LAi_CreateFantomGroup("Soldiers", 5, LAI_GROUP_ENEMY, LAI_GROUP_ENEMY, FRANCE, OFFIC_TYPE_GUARD, 0, true, "", "", "", "");}
			// Julian: for avoiding landingboat location <--
