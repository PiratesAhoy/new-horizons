...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			//B. -->
			if(FindFirstContrabandGoods(PChar) != -1 && Pchar.location.from_sea == "Douwesen_shore_01" && Pchar.quest.Contraband.Active != true)
			{
		...
		case "Smuggling_exit":
			...
			DialogExit();
			NPChar.greeting = "Gr_Pieter Nolt";
		break;
		//B. <--
	}
}
