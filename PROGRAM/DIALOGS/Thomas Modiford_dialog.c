void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			dialog.text = DLG_TEXT[0] + npchar.name + " " + npchar.lastname + DLG_TEXT[1] + GetMyAddressForm(NPChar, PChar, ADDR_CIVIL, false, false) + DLG_TEXT[2];		// changed from .spa to .por by KAM
		...
		case "node_1":
			...
			link.l1 = DLG_TEXT[9];
			link.l1.go = "quests";
			link.l2 = DLG_TEXT[10];		// + GetCharacterAddressForm(NPChar, ADDR_TITLE, false, false) + ".";	// LDH 23Apr09
			link.l2.go = "node_2";
		break;
