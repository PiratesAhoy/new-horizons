//#include "DIALOGS\Robert LeNoir_dialog.h"
...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "begin_4":
			...
			dialog.text = DLG_TEXT[13] + (makeint(Npchar.rank)*5000) + DLG_TEXT[49];
			link.l1 = DLG_TEXT[14];
			link.l1.go = "begin_6";
			link.l2 = DLG_TEXT[15];
			link.l2.go = "exit";
		...
		case "wait_1":
			...
			DialogExit();
			WaitDate("", 0,0,1,0,0);
			Log_SetStringToLog(LanguageConvertString(tmpLangFileID,"You spend 1 day in the house") + ".");
		...
		case "confirm":
			dialog.snd = "Voice\HAAU\HAAU005";
			dialog.text = DLG_TEXT[39] + (makeint(Npchar.rank)*5000) + DLG_TEXT[52];
			link.l1 = DLG_TEXT[40];
