void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "quest lines":
			...
			Link.l1 = DLG_TEXT[18];
			Link.l1.go = "no quest";
        	if (CheckQuestAttribute("BadBlaze", "partir"))
        	{
		       link.l2 = DLG_TEXT[34];
		       link.l2.go = "ship_1";
	        }
		...
		case "shipyard":
			...
			d.Text = DLG_TEXT[30] + DLG_TEXT[24];	// ccc 15oct04 shipyard waiting
			Link.l1 = DLG_TEXT[24];
		...
		case "ship_3":
			...
			link.l1 = DLG_TEXT[40];
			link.l1.go = "exit";
			pchar.quest.BadBlaze = "voirautre";
			AddQuestRecord("Bad_Blaze", "2");
		break;
	}
}
