//#include "DIALOGS\Roche at QC_dialog.h"
...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			dialog.text = DLG_TEXT[2] + GetMyFullName(PChar) + DLG_TEXT[0];
			link.l1 = DLG_TEXT[3];
			link.l1.go = "begin";
		...
		case "wait_1":
			...
			DialogExit();
			WaitDate("", 0,0,1,0,0);
			Log_SetStringToLog(LanguageConvertString(tmpLangFileID,"You spend 1 day in Quebradas Costillas") + ".");
		...
		case "partnership":
			...
			link.l1 = DLG_TEXT[43];
			link.l1.go = "exit_2";
			Group_SetAddressNone("Roche_Ship");			
			SetCompanionIndex(pchar, -1, getcharacterIndex(npchar.id));
			SetCharacterRemovable(npchar, false);
			npchar.location = "QC_Tavern";
			npchar.location.group = "goto";
