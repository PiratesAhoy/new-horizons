//#include "DIALOGS\Hugh Chadman_dialog.h"
void ProcessDialogEvent()
{
	...
	// -----------------------------------Диалог первый - первая встреча
		case "First time":
			...
			if(TradeCheck(PChar, NPChar, true)) { // NK	// LDH added true 03Jan09
				...
				else
				{
					...
					d.Text = TimeGreeting() + DLG_TEXT[5] + PChar.name + DLG_TEXT[6] + GetMyAddressForm(NPChar, PChar, ADDR_CIVIL, false, false) + DLG_TEXT[7];
					Link.l1 = DLG_TEXT[8];
					Link.l1.go = "Shipyard";
					Link.l2 = DLG_TEXT[9];
		...
		case "shipyard":
			...
			d.Text = DLG_TEXT[30] + " " + DLG_TEXT[16] + PChar.name + DLG_TEXT[17];	// ccc 15oct04 shipyard waiting : changed by MAXIMUS
			Link.l1 = DLG_TEXT[24];
		...
		case "eglise":
			dialog.snd = "Voice\DAFA\DAFA004";
 			...
 			if (CheckQuestAttribute("BadBlaze", "voirhugh"))
			{			
				link.l2 = DLG_TEXT[34];
				link.l2.go = "eglise_1";
			}
		...
		case "eglise_2":
			...
			link.l1 = DLG_TEXT[38];
			link.l1.go = "exit";			
			pchar.quest.BadBlaze = "voirchurch";
                       AddDialogExitQuest("natcharles6");
