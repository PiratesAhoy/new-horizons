void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "node_2":
			...
			link.l1 = pcharrepphrase(DLG_TEXT[6] + GetMyName(PChar) + DLG_TEXT[7], DLG_TEXT[8] + GetMyFullName(PChar) + DLG_TEXT[9]);
...
//Hitman - disabled by Grey Roger because it happens at Point a Pitre
...
//Bartolomeu's trap
			if(CheckQuestAttribute("trap", "grenada"))
			{
				link.l3 = DLG_TEXT[89];
				link.l3.go = "begin_1";
			}			
...
//Bartolomeu's trap
		...
		case "begin_4":
			...
			link.l1 = DLG_TEXT[97];
			link.l1.go = "exit";
			AddDialogExitQuest("Bart_grenadabis2");			
		break;		
