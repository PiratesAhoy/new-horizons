...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if(checkquestattribute("Bonnie_talk", "Needing_some_help") && checkquestattribute("Petros_contract", "open"))
			{
				dialog.text = DLG_TEXT[2];
				link.l1 = DLG_TEXT[3];			
				link.l1.go = "Island_stuff";
			}
			if (checkquestattribute("Bonnie_talk", "Hunting_for_Blaze") && checkquestattribute("Chosen_Quest", "Petros"))
			{
			...
			if(checkquestattribute("Bonnie_talk", "Needing_some_help") && checkquestattribute("Petros_contract", "sealed"))
			{
		...
		case "Exit_Island_stuff":
			...
			pchar.quest.Chosen_Quest = "Petros";
			DialogExit();
		...
		case "Exit_for_choice":
			pchar.quest.Petros_contract = "sealed";
			DialogExit();
			NextDiag.CurrentNode = NextDiag.TempNode;
		break;		
