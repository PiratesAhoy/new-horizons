...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if(checkquestattribute("Bonnie_talk", "Needing_some_help") && checkquestattribute("CCC_contract", "open"))
			{
				dialog.text = DLG_TEXT[2];
				link.l1 = DLG_TEXT[3];			
				link.l1.go = "Island_stuff";
			}
			if (checkquestattribute("Bonnie_talk", "Hunting_for_Blaze") && checkquestattribute("Chosen_Quest", "CCC"))
			{
			...
			if(checkquestattribute("Bonnie_talk", "Needing_some_help") && checkquestattribute("CCC_contract", "sealed"))
			{
