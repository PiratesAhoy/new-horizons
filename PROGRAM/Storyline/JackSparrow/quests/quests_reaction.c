...
//			DoQuestReloadToLocation(QDeck, "rld", "startloc", "Tia_to_Davy2");	// Doesn't work if you direct-sail to next island
	   ...
	   //Go to Elton Garfield on docks
		...
		//cannibals for ambush
		case "spawn_cannibals_2":
			sld = LAi_CreateFantomCharacter(false, 1, true, false, 1.0, "RabBlack", "monsters", "monster1");
			sld.name = TranslateString("", "Cannibal");
			sld.lastname = "";
			sld.money = 0;
			LAi_SetHP(sld, 80.0, 80.0);
			...
			sld = LAi_CreateFantomCharacter(false, 1, true, false, 1.0, "RabBlack_1", "goto", "goto5");
			sld.name = TranslateString("", "Cannibal");
			sld.lastname = "";
			sld.money = 0;
			TakeItemFromCharacter(sld, CheckCharacterEquipByGroup(sld, BLADE_ITEM_TYPE));
			...
			sld = LAi_CreateFantomCharacter(false, 1, true, false, 1.0, "RabBlack", "goto", "goto4");
			sld.name = TranslateString("", "Cannibal");
			sld.lastname = "";
			sld.money = 0;
			TakeItemFromCharacter(sld, CheckCharacterEquipByGroup(sld, BLADE_ITEM_TYPE));
		...
		//Cannibal runs toward Jack out of cover
		case "attack_cannibals_2":
			LAi_ActorRunToLocator (characterFromID("Cannibal_j"), "goto", "Goto10", "", 31);
			LAi_QuestDelay("attack_cannibals_3", 0.7);
		break;

		//Turn to party/fire bow animations
		case "attack_cannibals_3":
			LAi_SetActorType(pchar);
			LAi_SetActorType(characterFromID("Cannibal_j"));
			LAi_SetActorType(characterFromID("Cannibal_g"));
			LAi_SetActorType(characterFromID("Cannibal_A"));

			LAi_ActorTurnToCharacter(characterFromID("Cannibal_j"), pchar);
			LAi_ActorTurnToCharacter(characterFromID("Cannibal_g"), characterFromID("Mr. Gibbs"));
			LAi_ActorTurnToCharacter(characterFromID("Cannibal_A"), characterFromID("Annamaria"));

			LAi_ActorAnimation(characterFromID("Cannibal_j"), "shot", "play_arrow_sounds_Jack", 0.7);
			LAi_ActorAnimation(characterFromID("Cannibal_g"), "shot", "play_arrow_sounds_Gibbs_Delay", 0.8);

			//We make a delay for Anna here so it looks more natural
			LAi_QuestDelay("Attack_Anna_delay", 0.4);
		break;

		case "play_arrow_sounds_Jack":
		   PlaySound("OBJECTS\DUEL\bow.wav");
		   LAi_ActorAnimation(pchar, "hit_fire", "", 0.2);
		   LAi_QuestDelay("attack_cannibals_poison", 1.5);
		break;

		//Delay so sounds don't overlap
		case "play_arrow_sounds_Gibbs_Delay":
		   LAi_QuestDelay("play_arrow_sounds_Gibbs", 0.1);
		break;

		case "play_arrow_sounds_Gibbs":
		   PlaySound("OBJECTS\DUEL\bow.wav");
		   LAi_ActorAnimation(characterFromID("Mr. Gibbs"), "hit_fire", "", 0.1);
		break;

		case "Attack_Anna_delay":
		   LAi_ActorAnimation(characterFromID("Cannibal_A"), "shot", "play_arrow_sounds_Anna", 0.7);
		break;

		case "play_arrow_sounds_Anna":
		   PlaySound("OBJECTS\DUEL\bow.wav");
		   LAi_ActorAnimation(characterFromID("Annamaria"), "hit_fire", "", 0.2);
		break;
		...
		case "attack_cannibals_poison":
			...
			LAi_QuestDelay("dialogue_gibbs_cave_poisoned", 0.2)
		break;
		...
		case "dialogue_gibbs_cave_poisoned":
			LAi_SetPlayerType(pchar);
			LAi_SetActorType(characterFromID("Mr. Gibbs"));
			LAi_ActorAnimation(characterFromID("Annamaria"), "Afraid", "", 15);
			Characters[GetCharacterIndex("Mr. Gibbs")].Dialog.Filename = "Mr. Gibbs_dialog.c";
		...
		case "TP_Indian_village":
			...
			LAi_QuestDelay("TP_Indian_village2", 0.1);
		break;

		case "TP_Indian_village2":
			DoQuestReloadToLocation("Indian_Village2", "goto", "goto4", "spawn_cannibals_3");
		break;
		...
		case "cleanup_after_roa":
			...
			//Kraken attacks if player takes more than 20 days to sail to Muscetto Cove
			PChar.quest.Kraken_attack_dmc_1.win_condition.l1 = "Timer";
			PChar.quest.Kraken_attack_dmc_1.win_condition.l1.date.day = GetAddingDataDay(0,0,20);
			PChar.quest.Kraken_attack_dmc_1.win_condition.l1.date.month = GetAddingDataMonth(0,0,20);
			PChar.quest.Kraken_attack_dmc_1.win_condition.l1.date.year = GetAddingDataYear(0,0,20);
		...
		//After smugglers resolved/fought
		...
		case "Video_to_shipwreck3":
			LAi_Fade("tp_deck_before_shipwreck_delay", "");
		break;

		case "tp_deck_before_shipwreck_delay":
			LAi_QuestDelay("tp_deck_before_shipwreck", 0.1);
		break;
		...
		case "Video_to_deck_from_shipw":
			if(LanguageGetLanguage() == "RUSSIAN") PostVideoAndQuest("LegendJackSparrow\RUSSIAN\DMC_dutchman_wreck_davey", 25, "Video_to_deck_from_shipw2");
			else PostVideoAndQuest("LegendJackSparrow\DMC_dutchman_wreck_davey", 25, "Video_to_deck_from_shipw2");
			SetCurrentTime(24.00, 0);
			SetNextWeather("Heavy Rain");
		break;
		...
		case "Gibbs_to_crewman2_tavern":
			//Goto goto3 for appearing //GOTO GOTO 2 to go
