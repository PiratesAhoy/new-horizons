...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if (CheckAttribute(PChar, "quest.imperial_escort.arrest_guzman") && sti(PChar.quest.imperial_escort.arrest_guzman) == true && LAi_IsDead(CharacterFromID("Emiliano de Guzmбn")))
			{
				link.l1 = XI_ConvertString(GetRankName(NPChar, sti(NPChar.nation))) + DLG_TEXT[179] + GetMyFullName(CharacterFromID("Emiliano de Guzmбn")) + DLG_TEXT[180];
...
//				PChar.quest.finale_assign_galleons = "true";
		...
		case "imperial_escort_whodunnit":
			...
			link.l1 = DLG_TEXT[90] + XI_ConvertString(GetRankName(NPChar, sti(NPChar.nation))) + DLG_TEXT[91] + GetMyFullName(CharacterFromID("Javier Balboa")) + DLG_TEXT[92] + GetMyFullName(CharacterFromID("Emiliano de Guzmбn")) + DLG_TEXT[93];
		...
		case "imperial_escort_why_believe":
			dialog.text = DLG_TEXT[96] + GetMyName(CharacterFromID("Emiliano de Guzmбn")) + DLG_TEXT[97];
			link.l1 = DLG_TEXT[98];
			link.l1.go = "exit";
		break;
		...
		case "imperial_escort_that_seems_fair":
			...
			dialog.text = DLG_TEXT[101] + DLG_TEXT[102] + GetMyFullName(CharacterFromID("Emiliano de Guzmбn")) + DLG_TEXT[103] + GetMyFullName(CharacterFromID("Javier Balboa")) + DLG_TEXT[104] + GetMyFullName(PChar) + DLG_TEXT[105] + ships + DLG_TEXT[108];
			if(LAi_IsDead(CharacterFromID("Emiliano de Guzmбn")))
			{
				link.l1 = DLG_TEXT[115] + GetMyFullName(CharacterFromID("Emiliano de Guzmбn")) + DLG_TEXT[116];
				AddDialogExitQuest("imperial_escort_restore_santiago_governor");
				link.l1.go = "exit";
			}
			else
			{
				link.l1 = XI_ConvertString(GetRankName(NPChar, sti(NPChar.nation))) + DLG_TEXT[110] + GetMyFullName(CharacterFromID("Emiliano de Guzmбn")) + DLG_TEXT[111];
