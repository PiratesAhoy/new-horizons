	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if(CheckAttribute(Pchar,"quest.pircap_perks") && Pchar.quest.pircap_perks == "coins_instead")
			{
				...
				{
					if(CheckCharacterItem(NPchar,"coin13") || CheckCharacterItem(NPchar,"coin14") || CheckCharacterItem(NPchar,"coin15"))
					{
		...
		case "coins":
			...
			itmid = "coin13";
			if(CheckCharacterItem(NPchar,"coin13"))
			{
			...
			itmid = "coin14";
			if(CheckCharacterItem(NPchar,"coin14"))
			{
				NPChar.itemtrade.itemlist.items.(itmid).qty 	= 1.0;
				NPChar.itemtrade.itemlist.items.(itmid).price 	= 1.25;
				NPChar.itemtrade.itemlist.items.(itmid).fprice 	= 1.25;	
			...
			itmid = "coin15";
			if(CheckCharacterItem(NPchar,"coin15"))
			{
				NPChar.itemtrade.itemlist.items.(itmid).qty 	= 1.0;
				NPChar.itemtrade.itemlist.items.(itmid).price 	= 1.25;
				NPChar.itemtrade.itemlist.items.(itmid).fprice 	= 1.25;	
