...
void LAi_type_lay_CharacterUpdate(aref chr, float dltTime)
{
	...
	if(bAbordageStarted) { snd = "seaman"; }
	if(CheckAttribute(chr,"sex") && chr.sex=="woman") { snd = "female-citizen"; }
//	if(CheckAttribute(chr,"sleep")) { snd = "PEOPLE\snore.wav"; }	// LDH should be trep\PEOPLE\snore.wav - 23Mar09
	if(CheckAttribute(chr,"sleep")) { snd = "snore"; }
