...
bool EndQuestMovie()
{
	//Trace("::: QUESTMOVIE: EndQuestMovie func call");// TIH bugtracking
	if(questMovieProcess == 0) return false;
	//Trace("::: QUESTMOVIE: EndQuestMovie func proceed");// TIH bugtracking
...
// TIH --> quest movie tracking Nov3'06
void TrackQuestMovie(string whatDoing,string whereFrom)
{
	// To turn off logging and tracing of each questmovie start/stop, simply 
	// comment out the below two lines (you must leave the function call in tact!)
	Trace("::: QUESTMOVIE: " + whatDoing + " in " + whereFrom);
	//LogIt("QUESTMOVIE: " + whatDoing + " in " + whereFrom);
